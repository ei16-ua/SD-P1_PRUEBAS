@echo off
REM ========================================
REM DRIVER WEB GUI - Inicio individual
REM ========================================
...
REM Solicitar puerto web (opcional)
...
set /p KAFKA_IP="IP de Kafka para telemetria (ej: 192.168.1.11 o 'localhost', enter=sin telemetria): "
...
echo ========================================
echo.
echo Abriendo navegador en http://localhost:%WEB_PORT%
echo.

REM Construir comando
set CMD=python src\EV_Driver\EV_Driver_Web.py --driver-id %DRIVER_ID% --central-host %CENTRAL_IP% --central-port 9099 --web-port %WEB_PORT%

if not "%KAFKA_IP%"=="" (
    set CMD=%CMD% --kafka-bootstrap %KAFKA_IP%:%KAFKA_PORT%
)

REM Iniciar el servidor
start "DRIVER-%DRIVER_ID%-WEB" %CMD%
...
echo.
echo ========================================
echo   DRIVER WEB GUI INICIADO
echo ========================================
echo.
echo Abre tu navegador en: http://localhost:%WEB_PORT%
echo.
